...
아 그냥 프롬프트를 지금처럼 거창하게 하지말고 예전에 사용할때처럼 해볼까 답변도 json말고 그냥 자연어로 txt로 저장하고 결과 테스트 해볼만한데
...
6. 도커 이미지로 올려버리면 편할듯? 지금 종속성 문제가 조금 있음 -> 그럼 codex 로그인은 어떻게? -> 자동화 or 그냥 첫 사용시 shell 붙어서 직접하게 하면?
